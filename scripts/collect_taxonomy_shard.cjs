...
async function collectCategoryListings(page, categoryId, pages, options = {}) {
  ...
  // The four small root categories can have more inventory in the normal
  // category search than in the bestseller endpoint. Sweep their result set
  // completely, but stop at the real end instead of inventing products.
  const priorityCategoryIds = new Set((options.priorityCategoryIds || []).map(Number));
  let priorityProducts = 0;
  let priorityPages = 0;
  if (priorityCategoryIds.has(Number(categoryId))) {
    const pageSize = Math.max(1, Number(options.prioritySearchPageSize || 36));
    const maxPages = Math.max(1, Number(options.prioritySearchMaxPages || 30));
    const before = categoryMemberships.length;
    for (let pageNumber = 1; pageNumber <= maxPages; pageNumber++) {
      const items = await searchFetch(page, categoryId, pageNumber, pageSize);
      priorityPages++;
      addItems(items, 'priority_category_search', (pageNumber - 1) * pageSize + 1);
      const totalPages = items.total ? Math.ceil(items.total / pageSize) : null;
      if (items.length < pageSize || (totalPages && pageNumber >= totalPages)) break;
      const pauseMs = Number(options.prioritySearchPauseMs || 0);
      if (pauseMs > 0) await sleep(pauseMs);
    }
    priorityProducts = categoryMemberships.length - before;
  }
...
async function collect() {
  ...
  const priorityCategoryIds = new Set((collectionConfig.priorityCategoryIds || []).map(Number));
  let expansionMemberships = 0; let expansionPageFailures = 0; let priorityPages = 0; let priorityProducts = 0;
  ...
  try {
    ...
    for (let categoryIndex = 0; categoryIndex < nodes.length; categoryIndex++) {
      ...
      if (categoryResult) {
        ...
        priorityPages += categoryResult.priorityPages || 0;
        priorityProducts += categoryResult.priorityProducts || 0;
      ...
      if ((categoryIndex + 1) % 10 === 0 || categoryIndex + 1 === nodes.length) {
        writeJsonAtomic(statusFile, {
          ...
          priorityCategories: priorityCategoryIds.size, priorityPages, priorityProducts,
  ...
  const result = {
    ...
    priorityPages, priorityProducts,
