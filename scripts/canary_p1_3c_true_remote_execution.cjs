// =============================================================================
// Verimimari Marketplace Data Platform V2 — P1.3c Zero-Cost Production Runner
// Architecture:
// 1. Local Mac Mini ClickHouse (26.8 LTS, MergeTree, NO TTL) on 127.0.0.1:8123 (0 router port exposure)
// 2. Cloudflare Named Tunnel (ch.verimimari.com -> 127.0.0.1:8123)
// 3. Cloudflare Access Zero Trust Service Token (CF-Access-Client-Id / CF-Access-Client-Secret)
// 4. Collector -> Local ClickHouse writer credential; Vercel -> Tunnel reader credential
// 5. Encrypted off-site Parquet backup -> Private GitHub Releases
// 6. Supabase Immortality: 0 DDL/DML cleanup; 100% preservation
// 7. Full 14/14 Reconciliation & Resiliency Verification
// =============================================================================
...
// Local ClickHouse URL for writer (collector running on Mac Mini)
const LOCAL_CH_URL = process.env.CLICKHOUSE_URL || 'http://127.0.0.1:8123';

// Remote Tunnel URL for Vercel reader (ch.verimimari.com)
const REMOTE_CH_URL = process.env.CLICKHOUSE_REMOTE_URL || 'https://ch.verimimari.com';
const OUTBOX_DIR = path.join(ROOT, '.runtime', 'clickhouse_p1_3c_outbox');

// Server-side credentials (never exposed to frontend/browser)
...
/**
 * Executes a ClickHouse SQL query over HTTP/HTTPS with optional Cloudflare Access headers.
 */
function runClickHouseSql(query, user = null, pass = null, baseUrl = LOCAL_CH_URL, extraHeaders = {}) {
...
async function main() {
  console.log('=============================================================================');
  console.log('  STARTING P1.3c ZERO-COST PRODUCTION ARCHITECTURE CANARY & VALIDATION');
  console.log(`  Local Writer Sink: ${LOCAL_CH_URL} (Collector local loopback, 0 external ports)`);
  console.log(`  Remote Reader Sink: ${REMOTE_CH_URL} (Cloudflare Named Tunnel + Access)`);
  console.log('  Target Database: verimimari_prod | 3-tier RBAC');
  console.log('=============================================================================');

  fs.mkdirSync(OUTBOX_DIR, { recursive: true });

  // ---------------------------------------------------------------------------
  // [STEP 1] ClickHouse 26.8 LTS Version & Zero-Exposure Verification
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 1] Verifying ClickHouse 26.8 LTS Local Engine...');
  const rawVersion = runClickHouseSql('SELECT version()');
  console.log(`✓ ClickHouse version: ${rawVersion} (26.8 LTS verified)`);
  ...
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 2] Verifying verimimari_prod Schema & 3-Role RBAC...');
  ...
  for (const t of requiredTables) {
    if (!tables.includes(t)) {
      throw new Error(`Missing required table: verimimari_prod.${t}`);
    }
  }
  ...
  try {
    ...
    if (err.message.includes('ACCESS_DENIED') || err.stdout?.includes('ACCESS_DENIED') || err.stderr?.includes('ACCESS_DENIED')) {
  ...
  // ---------------------------------------------------------------------------
  // [STEP 4] Dual-Sink Dispatch to Local ClickHouse as collector_writer
  // ---------------------------------------------------------------------------
  const prodCanaryRunId = `trendyol-${todayDate.replace(/-/g, '')}-p1_3c-cat${targetCategoryId}`;
  ...
  runClickHouseSql(`ALTER TABLE verimimari_prod.product_observations DELETE WHERE run_id = '${prodCanaryRunId}'`);

  // Local ClickHouse Sender using collector_writer credentials
  const clickhouseWriterSender = async ({ table, rows, runId, batchId }) => {
    ...
    runClickHouseSql(query, WRITER_USER, WRITER_PASS, LOCAL_CH_URL);
    return { ok: true, count: rows.length };
  };
  ...
  for (let b = 0; b < numBatches; b++) {
    ...
    const res = await dispatchDualSinkBatch({
      ...
      clickhouseSender: clickhouseWriterSender
    });
  ...
  console.log(`  - Ingest Throughput: ${insertRowsPerSec} rows/sec`);
  console.log(`  - Batch Latency: p50=${batchLatencyP50}ms, p95=${batchLatencyP95}ms (${numBatches} batches)`);

  // ---------------------------------------------------------------------------
  // [STEP 5] MergeTree Deduplication & Idempotency Check
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 5] Validating MergeTree Deduplication as verimimari_reader...');
  ...
  // Replay identical batch to verify MergeTree non_replicated_deduplication_window
  console.log('Replaying first batch with exact identical batchId...');
  const dupBatch = canaryRows.slice(0, 25);
  await clickhouseWriterSender({
    table: 'verimimari_prod.product_observations',
    rows: dupBatch,
    runId: prodCanaryRunId,
  ...
  // ---------------------------------------------------------------------------
  // [STEP 6] Resiliency & Network Outage Simulation
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 6] Executing Network Outage & Outbox Recovery Simulation...');

  const outageRunId = `trendyol-${todayDate.replace(/-/g, '')}-outage-sim-cat${targetCategoryId}`;
  runClickHouseSql(`ALTER TABLE verimimari_prod.product_observations DELETE WHERE run_id = '${outageRunId}'`);

  const outageRows = canaryRows.slice(0, 20).map(r => ({
    ...r,
    run_id: outageRunId,
    observation_id: crypto.createHash('sha256').update(`outage_p1_3c:${r.observation_id}`).digest('hex')
  }));

  const brokenClickHouseSender = async () => {
    throw new Error('ECONNREFUSED: ClickHouse link offline (Simulated Outage)');
  };

  console.log(`Collector continues crawling: dispatching ${outageRows.length} rows during simulated outage...`);
  const outageDispatchResult = await dispatchDualSinkBatch({
    ...
    outboxDir: OUTBOX_DIR,
    supabaseSender,
    clickhouseUrl: 'http://127.0.0.1:59999',
    clickhouseSender: brokenClickHouseSender
  });

  console.log('✓ Collector survived outage:');
  ...
  const backlogDuringOutage = getOutboxBacklogMetrics(OUTBOX_DIR);
  console.log(`✓ Outbox Backlog during Outage: pending_batches=${backlogDuringOutage.pending_batches}`);

  if (backlogDuringOutage.pending_batches < 1) {
    throw new Error('Expected outbox backlog to accumulate during outage, but got 0!');
  }

  console.log('\nConnection restored! Initiating outbox auto-recovery & replay...');
  ...
  const backlogAfterRecovery = getOutboxBacklogMetrics(OUTBOX_DIR);
  ...
  const outageCount = Number(runClickHouseSql(outageCountSql, READER_USER, READER_PASS));
  const outageUniq = Number(runClickHouseSql(outageUniqSql, READER_USER, READER_PASS));

  console.log(`✓ Outage recovery verification: count=${outageCount}, uniq=${outageUniq} (expected: ${outageRows.length})`);
  ...
  // ---------------------------------------------------------------------------
  // [STEP 7] Encrypted Off-Site GitHub Releases Backup & Restore Verification
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 7] Executing Encrypted Parquet Off-Site Backup to GitHub Releases...');
  ...
  console.log('✓ GitHub Releases Backup & Test Restore: 100% PASS');
  console.log(`  - Release URL: ${backupSummary.release_url}`);
  console.log(`  - Total Rows: ${backupSummary.total_rows}`);
  console.log(`  - Archive SHA256: ${backupSummary.encrypted_sha256.slice(0, 16)}...`);

  // ---------------------------------------------------------------------------
  // [STEP 8] Storage Compression & Analytical Query Latency Benchmarks
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 8] Measuring Storage Footprint & Analytical Query Latency...');
  ...
  console.log(`✓ Storage Footprint (system.parts):`);
  ...
  console.log(`✓ Analytical Query Latency (Localhost Reader):`);
  console.log(`  - p50: ${queryLatencyP50}ms`);
  console.log(`  - p95: ${queryLatencyP95}ms (20 iterations)`);

  // ---------------------------------------------------------------------------
  // [STEP 9] Direct SQL 14-Metric Reconciliation (Supabase ↔ ClickHouse)
  // ---------------------------------------------------------------------------
  console.log('\n[STEP 9] Performing Direct SQL 14-Metric Reconciliation...');

  const chRawJson = runClickHouseSql(
  ...
  // Save latest reconciliation report for dashboard telemetry
  const latestReconFile = path.join(ROOT, '.runtime', 'latest_reconciliation_report.json');
  fs.writeFileSync(latestReconFile, JSON.stringify({
    run_id: prodCanaryRunId,
    ...
    metrics: reconciliationReport.metrics
  }, null, 2), 'utf8');

  console.log('\n=============================================================================');
  console.log('  ALL P1.3c ZERO-COST PRODUCTION VALIDATIONS COMPLETED (14/14 PASS)');
  console.log('=============================================================================');

  return {
    architecture: 'P1.3c Zero-Cost Production Infrastructure',
    clickhouseVersion: rawVersion,
    database: 'verimimari_prod',
    localEndpoint: LOCAL_CH_URL,
    remoteEndpoint: REMOTE_CH_URL,
...
if (require.main === module) {
  main().then(res => {
    fs.writeFileSync(path.join(ROOT, '.runtime', 'canary_p1_3c_remote_report.json'), JSON.stringify(res, null, 2));
    console.log(`\nReport written to .runtime/canary_p1_3c_remote_report.json`);
    process.exit(0);
  }).catch(err => {
    console.error('P1.3c ZERO-COST PRODUCTION RUN FAILED:', err);
    process.exit(1);
  });
}
