...
Taksonomi shard başına 100 kalıcı takip ürünü, 700 dönüşümlü yeni ölçüm ve önceki günün en çok 700 stok karşılaştırmasını toplar. Dönüşüm önce hiç detay ölçülmemiş ürün döndüren kategorileri, ardından hiç ölçülmemiş ürünleri ve son olarak en eski ölçümleri seçer. İlk ölçüm ertesi gün yeniden ziyaret edilerek 18–30 saatlik satış tahmini penceresi oluşturulur. Kalıcı URL listesi sıralamadan çıkan ürünlerin de tekrar izlenmesini sağlar. Önceki ölçümler `.runtime/inventory/shard-N.json` içinde, kapsama geçmişi 365 gün tutulur; bu dizin yedeklenmelidir. Silinirse yeni baz ölçümü gerekir. Kategori kapsamı, tarihsel detay kapsamı ve o günkü sayısal stok kapsamı ayrı raporlanır. Yaklaşık 113 bin mevcut ürünün ilk baz ve karşılaştırma turu, ürün evreni sabit kalır ve istekler başarılı olursa yaklaşık 41 günde tamamlanır. Tüm katalogdaki bütün satıcı/varyantların günlük tam stok kapsamı bu bütçeyle sağlanmaz; kapasite `taxonomy/collection-config.json` ile ayarlanır.

Çok Satanlar servisi bir kategoride sıfır ürün döndürürse normal kategori arama servisi aynı kategori kimliğiyle ve `BEST_SELLER` sırasıyla devreye girer. Sponsor mağaza kartları sonuç havuzuna eklenmez; yalnız servis içindeki kategori ürün dizisi kullanılır. Günlük/derin sayfa bütçesi korunur. Kurtarılan kategoriler önce en az bir detay ölçümü alır; ardından bu kaynaktan bulunan diğer ürünler genel katalog dönüşümünden önce tamamlanır ve ertesi gün stok karşılaştırma kuyruğuna alınır. İki kaynakta da ürün bulunmayan kategori raporda gerçek boş olarak kalır.
...
## Kısmi veya başarısız koşulda veri koruma

Collector koşunun sonunda kalite kapısı geçse de geçmese de eldeki kısmi sonuçları `data/backups/YYYY-MM-DD/<run-id>/` altında ayrı bir recovery yedeğine yazar. Yedekte manifest, liste sonucu, tamamlanan detay sonuçları ve varsa ürün JSON/CSV bulunur. Fatal hata, timeout veya SIGTERM durumunda sinyal işleyicisi aynı yedeği koşu kapanmadan önce oluşturur. Başarısız koşu son geçerli `latest` ve snapshot verisinin üzerine yazmaz; Hermes ikinci deneme de başarısız olursa recovery klasörünü GitHub ana dalına yayınlar.

Ana JSON, CSV, rapor ve kalite çıktıları geçici dosyaya yazılıp atomic rename ile yer değiştirilir; böylece dosya yazımı sırasında oluşan kesinti mevcut dosyayı yarım bırakmaz. Recovery yedekleri koşu sonunda bir kez yazılır; ürün başına anlık checkpoint kullanılmaz.
